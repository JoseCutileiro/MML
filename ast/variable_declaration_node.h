...
namespace mml
{
  ...
  public:
    ...
    variable_declaration_node(int lineno, std::shared_ptr<cdk::basic_type> varType, const std::string &identifier,
                              cdk::expression_node *initializer) : cdk::typed_node(lineno), _identifier(identifier), _initializer(initializer)
    {
      type(varType);
    }
